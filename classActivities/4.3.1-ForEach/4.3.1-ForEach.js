...
const data = [
  ...
  const cities =  data.forEach(user => console.log(user.address.city));
  // create an array of latitude and longitude coordinate pairs
  const coords = [];
  console.log("coords before append", coords);
  data.forEach(user => {
  ...
  const jargon = [];
  data.forEach(user => {
      let jargonString = `${user.company.catchPhrase} ${user.company.bs}`;
      jargon.push(jargonString);
  }
