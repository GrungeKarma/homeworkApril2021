...
let usersToDoList = [];
function usersToDo(arr, userID) {
  arr.forEach(element => {
    if (element.userId == userID && element.completed === false) {
      usersToDoList.push(
        `User# ${element.userId} Task # ${element.id} Details: "${element.title}"`
      );
    }
  });
}
...
const toDoObject = { ...usersToDoList };
